<template>
	<ul class="sort-tags list-unstyled clearfix">
	    <li>
	      <a :class="{'active':(options.sortName == 'newer')}" @click.prevent="changeSort('newer')" href="javascript:;">最新</a>
	    </li>
	    <li>
	      <a :class="{'active':(options.sortName == 'viewDown')}" @click.prevent="changeSort('viewDown')" href="javascript:;">热门</a>
	    </li>
      <li v-for="tag in tagList" :key="tag.id">
        <a :class="{'active':(options.tagId == tag.id)}" @click.prevent="changeTag(tag.id)" href="javascript:;">{{tag.name}}</a>
      </li>
		  <li>
